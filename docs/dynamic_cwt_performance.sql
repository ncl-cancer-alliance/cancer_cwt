CREATE OR REPLACE DYNAMIC TABLE DEV__MODELLING.CANCER__CWT_PATHWAY.CWT_PERFORMANCE (
	
	--Description: Performance data for each metric (CWT Pathway standard)
    --Author: Jake Kealey
...
FROM (
	--2WW
	SELECT * FROM DEV__MODELLING.CANCER__CWT_PATHWAY.CWT_PERFORMANCE_2WW
	UNION ALL
	--FDS
	SELECT * 
	FROM DEV__MODELLING.CANCER__CWT_PATHWAY.CWT_PERFORMANCE_FDS 
	UNION ALL
...
LEFT JOIN DEV__MODELLING.CANCER__CWT_PATHWAY.CWT_PATHWAY pw
...
--31 Day
SELECT * FROM DEV__MODELLING.CANCER__CWT_PATHWAY.CWT_PERFORMANCE_31DAY_FIRST
UNION ALL
